...
return new class extends Migration {
    public function up()
    {
        Schema::table('scholarship_profiles', function (Blueprint $table) {
            if (!Schema::hasColumn('scholarship_profiles', 'contact_no_2')) {
                $table->string('contact_no_2', 50)->nullable()->after('contact_no');
            }
            if (!Schema::hasColumn('scholarship_profiles', 'guardian_name')) {
                $table->string('guardian_name', 100)->nullable()->after('mother_name');
            }
            if (!Schema::hasColumn('scholarship_profiles', 'guardian_relationship')) {
                $table->string('guardian_relationship', 50)->nullable()->after('guardian_name');
            }
            if (!Schema::hasColumn('scholarship_profiles', 'guardian_contact_no')) {
                $table->string('guardian_contact_no', 50)->nullable()->after('guardian_relationship');
            }
            if (!Schema::hasColumn('scholarship_profiles', 'guardian_occupation')) {
                $table->string('guardian_occupation', 100)->nullable()->after('guardian_contact_no');
            }
            if (!Schema::hasColumn('scholarship_profiles', 'parents_guardian_gross_monthly_income')) {
