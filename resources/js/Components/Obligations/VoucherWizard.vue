...
import { useSystemOptions, getSystemOptionLabel, refreshSystemOptions } from '@/composables/useSystemOptions';
...
const selectedObrTypeOption = computed(() => {
    const currentValue = normalizedObrType.value;
    if (!currentValue) {
        return null;
    }

    return _obrTypeRaw.value.find(option => normalizeObrTypeValue(option.value) === currentValue) || null;
});
...
const formatParticularPrograms = (particular) => {
    return getParticularPrograms(particular)
        .map((program) => program?.shortname || program?.name || program)
        .filter(Boolean)
        .join(', ');
};

const formatParticularDescription = (particular) => {
    return String(particular?.description ?? '').trim();
};

const formatParticularAllotment = (particular) => {
    if (particular?.allotment === null || particular?.allotment === undefined || particular?.allotment === '') {
        return '';
    }

    return formatCurrency(particular.allotment);
};

const autoGeneratedParticularsDescription = ref('');
const autoGeneratedExplanation = ref('');
const normalizedPreviewParticularsDescription = computed(() => normalizeDocumentHtml(voucherData.obligations.particulars_description));
const normalizedPreviewExplanation = computed(() => normalizeDocumentHtml(voucherData.disbursements.explanation));
...
const buildDisbursementTypeTemplateContext = () => {
    const semesterAcademicYear = joinNonEmpty(particularsSemester.value, particularsAcademicYear.value);
    const payee = getPayeeDisplay() || voucherData.obligations.payee_name || '';

    const values = {
        course: particularsCourse.value,
        school: particularsSchool.value,
        year_level: particularsYearLevel.value,
        semester: particularsSemester.value,
        academic_year: particularsAcademicYear.value,
        semester_academic_year: semesterAcademicYear,
        grant_provision: particularsGrantProvision.value,
        payee,
        particulars_name: voucherData.obligations.particulars_name,
    };

    return Object.entries(values).reduce((context, [key, value]) => {
        const rawValue = withFallback(value);
        context[key] = rawValue.toUpperCase();
        context[`${key}_raw`] = rawValue;
        return context;
    }, {});
};

const replaceDisbursementTemplateTokens = (template, context) => {
    return String(template ?? '').replace(/{{\s*([a-zA-Z0-9_]+)\s*}}|{\s*([a-zA-Z0-9_]+)\s*}/g, (match, doubleBraceToken, singleBraceToken) => {
        const token = doubleBraceToken || singleBraceToken;
        return Object.prototype.hasOwnProperty.call(context, token) ? context[token] : match;
    });
};

const hasRenderableDisbursementTemplateTokens = (value) => {
    if (!value) {
        return false;
    }

    const template = String(value);
    const context = buildDisbursementTypeTemplateContext();
    return replaceDisbursementTemplateTokens(template, context) !== template;
};

const renderDisbursementTypeTemplate = (template) => {
    if (!template) {
        return '';
    }
    ...
    const renderedTemplate = replaceDisbursementTemplateTokens(template, context);

    return normalizeDocumentHtml(renderedTemplate);
};
...
watch(
    [
        ...
        () => selectedObrTypeOption.value?.particulars_template,
    ...
    () => {
        ...
        if (!currentDescription
            || currentDescription === autoGeneratedParticularsDescription.value
            || hasRenderableDisbursementTemplateTokens(currentDescription)) {
...
watch(
    [
        () => voucherData.obligations.obr_type,
        () => selectedObrTypeOption.value?.explanation_template,
        () => voucherData.disbursements.course,
        () => voucherData.disbursements.los_course,
        () => voucherData.disbursements.semester,
        () => voucherData.disbursements.academic_year,
    ...
    () => {
        ...
        if (!currentExplanation
            || currentExplanation === autoGeneratedExplanation.value
            || hasRenderableDisbursementTemplateTokens(currentExplanation)) {
...
watch(() => props.visible, async (newVal) => {
    ...
    if (newVal) {
        await refreshSystemOptions('disbursement_type');
    }
});
...
<template>
    <IosModal v-model:visible="isOpen" :title="getStepTitle()" :modal-content-style="wizardModalStyle"
        ...
        <template #header-left>
            <button class="ios-nav-btn ios-nav-cancel text-nav" @click="closeWizard">
                <AppIcon name="times" />
            </button>
        </template>
        <template #header-right>
            <button v-if="step === 5" class="ios-nav-btn ios-nav-action text-nav" @click="handleSubmit" :disabled="loading">
        ...
        <template #footer>
            <button class="ios-footer-btn" :disabled="step === 1" @click="previousStep">
                <AppIcon name="chevron-left" :size="14" /> Back
            </button>
            <button v-if="step < 5" class="ios-footer-btn ios-footer-btn-primary" @click="nextStep"
        ...
        <div ref="wizardContentRef" class="ios-body ios-compact-controls [&_.p-inputtext]:text-compact [&_.p-select]:text-compact [&_.p-select-label]:text-compact [&_.p-button:not(.p-button-icon-only)]:text-compact">
            <div class="space-y-4 short:space-y-2" style="padding-top: 12px; padding-bottom: 24px;">
                ...
                <div v-if="step === 1" class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <!-- Left: Search & Select -->
                    <div class="bg-white border border-[#e5e5ea] rounded-2xl p-4 space-y-3 h-full flex flex-col">
                        ...
                        <!-- Search Input -->
                        <div class="relative">
                            <InputText v-model="searchQuery" type="text" placeholder="Search by name..." class="w-full"
                                :disabled="!programFilter && scholars.length === 0" />
                        ...
                        <!-- Error Message -->
                        <div v-if="error" class="p-3 bg-red-50 border border-red-200 rounded-2xl text-red-700 text-sm">
                            {{ error }}
                        </div>
                    ...
                    <div
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Transaction
                                Status</label>
                            <Select v-model="voucherData.summary.transaction_status" :options="transactionStatusOptions"
                        ...
                        <div class="space-y-4 bg-white border border-[#e5e5ea] rounded-2xl p-4">
                            ...
                            <div>
                                ...
                                <Select v-model="selectedParticularId"
                                    ...
                                    placeholder="Select Particular" class="w-full particulars-select">
                                    <template #option="{ option }">
                                        <div class="flex flex-col gap-0.5 py-1">
                                            ...
                                            <p v-if="formatParticularPrograms(option)" class="text-xs text-gray-400">
                                                {{ formatParticularPrograms(option) }}
                                            </p>
                                        </div>
                                    </template>
                                    <template #value="{ value }">
                                        <template v-if="selectedParticular">
                                            ...
                                            <span v-if="formatParticularAllotment(selectedParticular)"
                                                class="text-gray-400">
                                                - {{ formatParticularAllotment(selectedParticular) }}
                                            </span>
                                            <span v-if="formatParticularDescription(selectedParticular)"
                                                class="text-gray-500">
                                                - {{ formatParticularDescription(selectedParticular) }}
                                            </span>
                                        </template>
                                        <template v-else>{{ voucherData.obligations.particulars_name || `Select
                                            Particular` }}</template>
                                    </template>
                                </Select>
                            </div>
                ...
                <div v-if="step === 3" class="space-y-4 text-gray-800">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        ...
                        <div class="bg-white border border-[#e5e5ea] rounded-2xl p-4 space-y-4">
                            ...
                            <div>
                                ...
                                <InputText v-model="voucherData.disbursements.year_level" type="text" class="w-full"
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        ...
                        <div class="bg-white border border-[#e5e5ea] rounded-2xl p-4">
                            ...
                            <Select v-model="voucherData.disbursements.grant_provision" :options="grantProvisionOptions"
                                optionLabel="label" optionValue="value" placeholder="Select grant provision"
                ...
                <div v-if="step === 4" class="space-y-4 text-gray-800">
                    <div class="bg-white border border-[#e5e5ea] rounded-2xl p-4">
                        ...
                        <div class="flex gap-3">
                            <button type="button" @click="voucherData.disbursements.type = 'disbursements'" :class="[
                            ...
                            <button type="button" @click="voucherData.disbursements.type = 'payroll'" :class="[
                                ...
                                <span :class="[
                                    ...
                                    <svg v-if="voucherData.disbursements.type === 'payroll'" class="w-3 h-3 text-white"
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 short:gap-2">
                        <div
                            ...
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Particulars
                                (Descriptions)</label>
                ...
                <div v-if="step === 5" class="space-y-4">
                    ...
                    <div class="bg-gray-50 p-4 rounded-2xl border border-gray-200">
                        ...
                        <div class="space-y-2 text-sm">
                            ...
                            <div v-if="voucherData.obligations.particulars_description"
                                ...
                                <div class="voucher-html-preview text-gray-700 text-sm"
                                    v-safe-html="normalizedPreviewParticularsDescription">
                                </div>
                            </div>
                            <!-- Disbursements Summary -->
                            <div v-if="voucherData.disbursements.explanation" class="pt-2 border-t border-gray-300">
                                <p class="text-gray-600 text-xs mb-2">{{ voucherData.disbursements.type ===
                                    'disbursements' ?
                                    'Disbursements' : 'Payroll' }} Explanation</p>
                                <div class="voucher-html-preview text-gray-700 text-sm"
                                    v-safe-html="normalizedPreviewExplanation">
                                </div>
                            </div>
                    ...
                    <!-- Notes Summary -->
                    <div v-if="voucherData.summary.notes" class="bg-yellow-50 p-4 rounded-2xl border border-yellow-200">
...
<style scoped>
:deep(.particulars-select .p-select-option.p-select-option-selected) {
    background: #ecfdf5 !important;
    color: #111827 !important;
}

:deep(.particulars-select .p-select-option.p-select-option-selected .text-gray-900),
:deep(.particulars-select .p-select-option.p-select-option-selected .text-gray-500),
:deep(.particulars-select .p-select-option.p-select-option-selected .text-gray-400) {
    color: #374151 !important;
}

:deep(.particulars-select .p-select-option.p-focus) {
