<template>
    <!--
        INCENTIVES PAYROLL — Landscape 13 × 8.5 in (936pt content width at 96 dpi)
        Copy of PayrollTemplate.vue with Course and School columns added after Names.
        Uses fixed-width flex columns measured in pt for easy manual layout.

        Column system (total ≈ 936pt usable at 6mm margin):
        ┌───────┬──────────────┬──────────────┬──────────────┬──────────┬──────────┬──────────────────┐
        │  No.  │    Names     │    Course    │    School    │ Yr Level │  Amount  │    Signature     │
        │ 40pt  │   220pt      │    130pt     │    130pt     │  80pt    │  100pt   │     flex:1       │
        └───────┴──────────────┴──────────────┴──────────────┴──────────┴──────────┴──────────────────┘
    -->
    <div style="display:flex;flex-direction:column;min-height:90vh;">
        ...
        <!-- Explanation title (text before "YAKAP" stays regular; "YAKAP" onward is bold/uppercase) -->
        <div v-if="explanationTitle.before || explanationTitle.highlight" style="padding:4pt 0;text-align:center;">
            <span class="t-11">{{ explanationTitle.before }}</span><span class="bold t-11">{{ explanationTitle.highlight }}</span>
        </div>

        <!-- Academic year / term (Dean's List only) -->
        <div v-if="academicYearLine" style="padding:2pt 0 6pt 0;text-align:left;margin-top:4pt;">
            <span class="bold t-10">{{ academicYearLine }}</span>
        </div>
        ...
        <div style="flex:1;display:flex;flex-direction:column;">
            ...
            <div class="dv-row b-l b-r" style="border-top:1pt solid #000;">
                <div class="bold t-11"
                    ...
                    No.</div>
                <div class="bold t-11"
                    style="width:220pt;border-right:1pt solid #000;display:flex;align-items:center;justify-content:center;padding:2pt;">
                    Names</div>
                <div class="bold t-11"
                    style="width:130pt;border-right:1pt solid #000;display:flex;align-items:center;justify-content:center;padding:2pt;">
                    Course</div>
                <div class="bold t-11"
                    style="width:130pt;border-right:1pt solid #000;display:flex;align-items:center;justify-content:center;padding:2pt;">
                    School</div>
                <div class="bold t-11"
                    style="width:80pt;border-right:1pt solid #000;display:flex;align-items:center;justify-content:center;padding:2pt;">
                    Year Level</div>
                <div class="bold t-11"
            ...
            <div v-for="(s, i) in scholars" :key="i" class="dv-row b-l b-r">
                <div class="bold t-10"
                    ...
                    {{ i + 1 }}</div>
                <div class="bold t-11"
                    style="width:220pt;border-right:1pt solid #000;display:flex;align-items:center;padding:2pt 4pt;">
                    {{ s.name }}</div>
                <div class="bold t-11"
                    style="width:130pt;border-right:1pt solid #000;display:flex;align-items:center;padding:2pt 4pt;">
                    {{ s.course || 'N/A' }}</div>
                <div class="bold t-11"
                    style="width:130pt;border-right:1pt solid #000;display:flex;align-items:center;padding:2pt 4pt;">
                    {{ s.school || 'N/A' }}</div>
                <div class="bold t-11"
                    ...
                    {{ s.year || 'N/A' }}</div>
                <div class="bold t-11"
                    style="width:100pt;border-right:1pt solid #000;display:flex;align-items:center;justify-content:flex-end;padding:2pt 8pt;">
                    {{ money(s.amount) }}</div>
            ...
            <div v-for="n in emptyRows" :key="'e' + n" class="dv-row b-l b-r">
                ...
                <div style="width:220pt;border-right:1pt solid #000;">&nbsp;</div>
                <div style="width:130pt;border-right:1pt solid #000;">&nbsp;</div>
                <div style="width:130pt;border-right:1pt solid #000;">&nbsp;</div>
...
/* ── explanation title: plain text before "YAKAP" stays regular, "YAKAP" onward is bold/uppercase ── */
const explanationTitle = computed(() => {
    const text = stripHtml(props.voucher.explanation || '').trim();
    const index = text.toUpperCase().indexOf('YAKAP');

    if (index === -1) {
        return { before: text, highlight: '' };
    }

    return {
        before: text.slice(0, index),
        highlight: text.slice(index).toUpperCase(),
    };
});

/* ── academic year / term line (Dean's List only) ── */
const academicYearLine = computed(() => {
    if (props.voucher.incentive_type !== 'deans_list') return '';

    const term = props.voucher.semester || '';
    const academicYear = props.voucher.academic_year || '';
    const suffix = [term, academicYear].filter(Boolean).join(' ');

    return suffix ? `FOR ACADEMIC YEAR ${suffix}` : '';
});
