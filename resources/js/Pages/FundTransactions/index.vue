...
} from "@/composables/useSystemOptions";
import {
    getStatusBadgeClass,
    getStatusTextClass,
    getStatusIcon,
...
const loadVoucherDocuments = async (voucherId) => {
    try {
        const response = await axios.get(
            `/api/fund-transactions/${voucherId}/documents`,
        );
...
const uploadFile = async (docType) => {
    if (!selectedVoucherForUpload.value || !uploadedFiles.value[docType])
        return;
    ...
    try {
        ...
        const docs = voucherDocuments.value.get(
            selectedVoucherForUpload.value.id,
        );
...
const previewDocument = async (docType) => {
    ...
    const doc = voucherDocuments.value.get(selectedVoucherForUpload.value.id)?.[
        docType
    ];
    if (!doc) {
    ...
    const downloadUrl =
        doc.download_url ||
        `/api/fund-transactions/${selectedVoucherForUpload.value.id}/document/${docType}/download`;
...
const showQrCode = async (voucher, docType = null) => {
    try {
        const response = await axios.post(
            `/api/fund-transactions/${voucher.id}/generate-qr`,
            {
                doc_type: docType,
            },
        );
...
const getFirstScholarNameFromCache = (voucher) => {
    ...
    const firstScholarId =
        typeof voucher.scholar_ids[0] === "object"
...
<template>
    ...
    <AdminLayout>
            ...
            >
                                ...
                                <div class="ios-section">
                                    ...
                                    <div class="ios-card px-4 py-3 space-y-2">
                                        ...
                                        >
                                            ...
                                            <Tag
                                                :value="
                                                    selectedVoucherDocuments.dv_payroll
                                                        ? 'Uploaded'
                                                        : 'Missing'
                                                "
                                                :severity="
                                        ...
                                        >
                                            ...
                                            <Tag
                                                :value="
                                                    selectedVoucherDocuments.cheque
                                                        ? 'Uploaded'
                                                        : 'Missing'
                                                "
                                                :severity="
                ...
                >
                    ...
                    >
                        ...
                        <Button
                            label="Upload"
                            @click="openFileUploadDialog(selectedVoucher)"
                            severity="warning"
                            outline
                            size="small"
                            class="flex-1 whitespace-nowrap !rounded-xl text-xs"
        ...
        <FileUploadModal
            ...
            @update:show="showFileUploadDialog = $event"
        >
            <div v-if="selectedVoucherForUpload" class="pt-4">
                <!-- Voucher Header -->
                <div class="ios-section">
                    <div
                        class="ios-card px-4 py-3 bg-blue-50 dark:bg-blue-950/30"
                    >
                        <p
                            class="text-2xs font-semibold text-blue-700 dark:text-blue-400 uppercase mb-1"
                        >
                            Voucher
                        </p>
                        <p
                            class="text-sm font-semibold text-[#1e3a5f] dark:text-blue-200"
                        >
                            {{ selectedVoucherForUpload.transaction_id }}
                        </p>
                        <p class="text-xs text-blue-500 mt-0.5">
                            {{ selectedVoucherForUpload.payee_name }}
                        </p>
                    </div>
                </div>
                ...
                <div class="ios-section">
                    ...
                    <div class="ios-card px-4 py-3">
                        <p class="text-sm text-gray-600 dark:text-gray-400">
                            Upload up to four documents: OBR, DV/Payroll, LOS,
                            and Cheque. Maximum 10MB per file.
                        </p>
                    </div>
                </div>
                        ...
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <div class="flex items-center gap-2">
                                <input
                                    ...
                                    type="file"
                                    accept=".pdf,.doc,.docx"
                                    @change="(e) => handleFileSelect('obr', e)"
                                    class="hidden"
                                />
                                <AppButton
                                    icon="folder-open"
                                    @click="$refs['fileInputs.obr'][0]?.click()"
                                    severity="info"
                                    text
                                    size="small"
                                    v-tooltip="'Select File'"
                                />
                                <AppButton
                                    icon="qrcode"
                                    @click="
                                        showQrCode(
                                            selectedVoucherForUpload,
                                            'obr',
                                        )
                                    "
                                    severity="info"
                                    text
                                    size="small"
                                    v-tooltip="'QR Code for OBR'"
                                />
                                <Badge
                                    v-if="
                                        uploadedFiles.obr ||
                                        voucherDocuments.get(
                                            selectedVoucherForUpload.id,
                                        )?.obr
                                    "
                                    value="✓"
                                    severity="success"
                ...
                <div class="ios-section">
                    <p class="ios-section-label text-compact">
                        DV/Payroll — Disbursement Voucher or Payroll
                    </p>
                    <div class="ios-card px-4 py-3.5">
                        <div class="flex items-center justify-between mb-3">
                            <div class="flex items-center gap-2">
                            ...
                            <div class="flex items-center gap-2">
                                <input
                                    ref="fileInputs.dv_payroll"
                                    type="file"
                                    accept=".pdf,.doc,.docx"
                                    @change="
                                        (e) => handleFileSelect('dv_payroll', e)
                                    "
                                    class="hidden"
                                />
                                <AppButton
                        ...
                        <div
                            v-if="uploadedFiles.dv_payroll"
                            class="mb-3 flex items-center justify-between bg-white dark:bg-[#272f38] p-2 rounded border border-gray-200 dark:border-white/10"
                        >
                            <p
                                class="text-xs text-gray-700 dark:text-gray-300 flex-1 truncate"
                            >
                                {{ uploadedFiles.dv_payroll.name }}
                            </p>
                        </div>
                        <div class="flex gap-2">
                            <AppButton
                            ...
                            <AppButton
                                ...
                                "
                                @click="previewDocument('dv_payroll')"
                                icon="eye"
                                severity="warning"
                                text
                                v-tooltip="'Preview'"
                            ...
                            <AppButton
                                ...
                                "
                                @click="downloadDocument('dv_payroll')"
                                icon="download"
                                severity="success"
                                text
                            />
                            <AppButton
                                ...
                                "
                                @click="removeDocument('dv_payroll')"
                                icon="trash"
                                severity="danger"
                                text
                            />
                        ...
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <div class="flex items-center gap-2">
                                <input
                                    ...
                                    type="file"
                                    accept=".pdf,.doc,.docx"
                                    @change="(e) => handleFileSelect('los', e)"
                                    class="hidden"
                                />
                                <AppButton
                                    icon="folder-open"
                                    @click="$refs['fileInputs.los'][0]?.click()"
                                    severity="info"
                                    text
                                    size="small"
                                    v-tooltip="'Select File'"
                ...
                <div class="ios-section mb-4">
                    <p class="ios-section-label text-compact">
                        Cheques — Cheque Copy or Payment Proof
                    </p>
                    <div class="ios-card px-4 py-3.5">
                        <div class="flex items-center justify-between mb-3">
                            <div class="flex items-center gap-2">
                            ...
                            <div class="flex items-center gap-2">
                                <input
                                    ref="fileInputs.cheque"
                                    type="file"
                                    accept=".pdf,.doc,.docx"
                                    @change="
                                        (e) => handleFileSelect('cheque', e)
                                    "
                                    class="hidden"
                                />
                                <AppButton
                                    icon="folder-open"
                                    @click="
                                        $refs['fileInputs.cheque'][0]?.click()
                                    "
                                    severity="info"
                                    text
                                    size="small"
        ...
        >
            <template #header>
                <div class="flex items-center justify-between w-full pr-2">
                    <span
                        class="font-semibold text-gray-900 dark:text-gray-100 text-base"
            ...
            <div v-if="previewData" class="pt-2">
                ...
                >
                    ...
                    <div class="ios-card px-4 py-2.5">
                        <div class="flex items-center gap-2">
                            <AppButton
                                icon="minus"
                                @click="previewZoom -= 10"
                                :disabled="previewZoom <= 50"
                                text
                                size="small"
                            />
                            <span
                                class="text-compact font-medium w-12 text-center"
                                >{{ previewZoom }}%</span
                            >
                            <AppButton
                                icon="plus"
                ...
                <div
                    v-else-if="
                        previewData.mimeType &&
                        previewData.mimeType.includes('pdf')
                    "
                    class="ios-section mb-4"
                >
                    <div class="ios-card p-4">
                        <p class="text-compact text-[#8E8E93] mb-3">
                            PDFs open in a new window for best viewing
                            experience
                        </p>
                        <AppButton
                            label="Open PDF in Viewer"
                            @click="
                        ...
                        <p class="text-xs text-[#8E8E93] mt-2">
                            Or use the Download button above to save to your
                            device
                        </p>
                    </div>
                </div>
                ...
                <div v-else class="ios-section mb-4">
                    <div class="ios-card px-4 py-8 text-center">
                        <AppIcon
                            name="file"
                            :size="48"
                            class="text-[#8E8E93] block mb-3"
                        />
                        <p
                            class="text-sm text-[#3c3c43] dark:text-gray-300 mb-1.5"
                        >
                            Preview not available for this file type
                        </p>
                        <p class="text-xs text-[#8E8E93] mb-4">
                            {{ previewData.mimeType || "File type unknown" }}
                        </p>
                        <AppButton
                            label="Download File"
                            @click="
