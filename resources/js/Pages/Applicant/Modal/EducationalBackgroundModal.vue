            ...
            <div class="fixed inset-0 overflow-y-auto">
                <div class="flex justify-center p-4 text-center items-center min-h-screen">
                    <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
                        ...
                        <DialogPanel
                            class="w-full max-w-lg transform overflow-hidden rounded  text-left align-middle shadow-xl transition-all">
                            <DialogTitle as="h3"
                                ...
                                <form @submit.prevent="submit" @keydown.enter.prevent>
                                    <div class="mt-4">
                                        <div class="w-full">
                                            ...
                                            <VueMultiselect v-model="form.level" :options="academic_levels"
                                                :close-on-select="true" class="mt-1" />
                                    ...
                                    <div class="mt-4 flex justify-between gap-4">
                                        <div class="w-full">
                                            ...
                                            <TextInput autofocus id="course" type="text"
                                                class="mt-1 block w-full uppercase"
                                                :disabled="form.level == 'ELEMENTARY' || form.level == 'SECONDARY' || form.level == ''"
                                                :class="form.level == 'ELEMENTARY' || form.level == 'SECONDARY' || form.level == '' ? 'bg-gray-100 cursor-not-allowed' : ''"
                                                v-model="form.course" />
...
const form = useForm({
    ...
    applicant_id: props.education?.applicant_id || props.applicant_id,
...
const submit = () => {
    if (props.action == 'add') {
        axios.post(route("applicants-api.addeducation"), { ...form })
            .then(response => {
    ...
    if (props.action == 'edit') {
        axios.put(route("applicants-api.updateeducation", form.id), form).then(response => {
            if (response.data.message === 'success') {
