# iOS Modal Layout Guide

Reference implementation: `resources/js/Pages/InterviewedApplicants/Modal/GenerateReportModalIOS.vue`

## Overview

A solid white/gray modal inspired by iOS Settings, built on **PrimeVue Dialog** in headless `#container` mode with custom pointer-based drag. Uses monochrome minimal button styling and opaque surfaces.
...
└── .ios-modal (solid shell, draggable via transform)
...
.ios-dialog-mask {
    /* Standard PrimeVue mask overlay */
}
</style>
```
...
|---|---|---|
| `.ios-modal` | `#F2F2F7` | Solid iOS gray, main shell |
| `.ios-nav-bar` | `#FFFFFF` | Solid white header, `0.5px solid #E5E5EA` bottom border |
| `.ios-card` | `#FFFFFF` | Solid white grouped card, `0.5px solid #E5E5EA` border |
| `.ios-segment-active` | `#FFFFFF` | Active tab, solid white |
| `.ios-destructive-btn` | `#FFFFFF` | Danger action, solid white |
